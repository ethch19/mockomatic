...
import { parseTime, Time, type TimeDuration } from '@internationalized/date';
import DropdownRunAction from "../DropdownRunAction.vue";
...
export const columns: ColumnDef<IRunPayload>[] = [
    ...
    },
    {
        ...
        cell: ({ row, table }) => {
            ...
            return h(TimeInput, {
                modelValue: time,
                onUpdate: (duration_diff) => {
                    table.options.meta?.updateData?.(row.index, 'scheduled_start', duration_diff);
                },
            });
        },
    },
    {
        ...
        cell: ({ row, table }) => {
            ...
            return h(TimeInput, {
                modelValue: time,
                onUpdate: (duration_diff) => {
                    table.options.meta?.updateData?.(row.index, 'scheduled_end', duration_diff);
                },
            });
        },
    },
    {
        id: "actions",
        cell: ({ row, table }) => {
            return h(DropdownRunAction, { run_index: row.index, slot_index: table.options.meta?.slot_index })
        }
    },
]